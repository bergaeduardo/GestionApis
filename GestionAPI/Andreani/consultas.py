...
SELECT TOP 200 NRO_PEDIDO, NUM_SEGUIMIENTO, TALON_PED
FROM SEIN_TABLA_TEMPORAL_SCRIPT
WHERE IMP_ROT = 1 
  AND NUM_SEGUIMIENTO IS NOT NULL 
  AND METODO_ENVIO != 'FLEX'
  AND (estadoIdEnvio not in('4','14','18','19','20') OR estadoIdEnvio IS NULL)
  AND (estadoEnvio not in('ENTREGADO','RESCATADO','COMPLETADO') OR estadoIdEnvio IS NULL)
ORDER BY ID DESC
"""
...
UPDATE EB_ENVIOS_WEB_DESDE_SUC 
SET estadoEnvio = ?, estadoIdEnvio = ?, fechaEstadoEnvio = ?
WHERE NUM_SEGUIMIENTO = ?
"""
